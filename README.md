...
Each form defines its own attachment data types. The data type name in the form-data request (e.g., `fileAttachment-MainLetter`) must match a data type defined in the form's application metadata. Retrieve available document types for a form:

```http
GET {{basePath}}/pat/{{app}}/api/options/doctypes
Authorization: Bearer {{altinnToken}}
```
